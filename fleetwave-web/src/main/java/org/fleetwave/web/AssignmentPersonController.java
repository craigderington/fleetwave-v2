...
public class AssignmentPersonController {
    ...
    public ResponseEntity<Assignment> assignToPerson(@RequestHeader("X-Tenant") String tenantId,
                                                     @RequestBody AssignToPersonRequest body) {
        Assignment a = assignmentService.assignToPerson(
            tenantId,
            body.getRadioId(),
            body.getPersonId(),
    ...
    public ResponseEntity<Assignment> returnFromPerson(@RequestHeader("X-Tenant") String tenantId,
                                                       @PathVariable("id") UUID assignmentId) {
        Assignment a = assignmentService.returnAssignment(tenantId, assignmentId, OffsetDateTime.now());
        return ResponseEntity.ok(a);
    }
}
